...
import { barTabItemsByTabId } from "../data/barTab.data";
import { BarPaymentStatus, BarTab, BarTabItem, BarTabStatus } from "../types/barTab.types";
...
export default function BarTabDetailsModal({ visible, tab, onClose }: Props) {
    ...
    const tabItems: BarTabItem[] = barTabItemsByTabId[tab.id] ?? [];
        ...
        <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
            <View className="flex-1" pointerEvents="box-none">
                ...
                <View className="absolute inset-x-0 top-0 bottom-16 flex-row">
                    ...
                    <View className="w-[80%] rounded-l-2xl px-4 py-4"
                    ...
                    >
                        <ScrollView showsVerticalScrollIndicator={false}>
                            ...
                            <ScrollView className="px-4" showsVerticalScrollIndicator={false}>
                                ...
                                <View className="mt-4 rounded-xl bg-card p-4">
                                    <View className="mb-4 flex-row items-center">
                                        ...
                                        <Text className="ml-2 text-xl font-semibold text-white">Tab Bar Items ({tabItemCount})</Text>
                                    </View>
                                        ...
                                        tabItems.map((item) => (
                                            <View key={item.id} className="mb-3 rounded-lg border-b bg-black pb-3">
                                                <View className="mb-2 flex-row items-center justify-between px-4 py-4">
                                                    <Text className="text-lg font-semibold text-yellow">{item.id}</Text>
                                                    <Text className="text-lg text-zinc-300">{item.name}</Text>
                                                </View>

                                                <View className="mb-1 flex-row items-center gap-2 px-4">
                                                    <Text className="text-xl text-zinc-400">Qty:</Text>
                                                    <Text className="text-xl text-white">{item.qty}</Text>
                                                    <Text className="text-xl text-zinc-400">{item.unit}</Text>
                                                </View>
                                                ...
                                                {item.note ? (
                                                    <View className="mt-1 flex-row items-center gap-2 px-4">
                                                        ...
                                                        <Text className="text-xl italic text-zinc-300">
                                                            {item.note}
                                                        </Text>
                                                    </View>
                                                ) : null}
                                ...
                                <View className="mt-4 rounded-xl bg-card p-4 mb-4">
                                    ...
                                    <View className="mb-2 flex-row justify-between pb-2">
                                        <Text className="text-xl text-zinc-300">
                                            SubTotal:
                                        </Text>
                                        <Text className="text-xl text-zinc-300">
                            ...
                            <View className="mt-2 px-4 pb-4 pt-3">
                                <Pressable
                                    className="rounded-lg bg-yellow py-3 mb-2"
                                >
                                ...
                                <Pressable
                                    className="rounded-lg bg-[#3a4455] py-3"
                                >
